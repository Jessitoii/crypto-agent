...
MIN_ROI_THRESHOLD = 0.5 # 0.5%
STOP_LOSS_LIMIT = 0.5
OUTPUT_FILE = "hold_data.jsonl"
COIN_MAP = get_top_100_map()

async def get_market_outcome(ctx, pair, msg_ts, btc_trend): 
    """Collects market data at the time of the news. Fast, excludes BTC query."""
    try:
        start_ms = int(msg_ts * 1000)
        
        # 1. Technical Analysis (100 min lookback)
        ...
        # 3. Post-News Outcome (20 min window)
        ...
        # Filtering logic for HOLD data generation
...
async def get_btc_trend(ctx, msg_ts):
    """Calculates 1-hour BTC trend at the time of the news."""
    try:
        start_ms = int(msg_ts * 1000)
                ...
                for i, message in enumerate(all_msgs):
                    ...
                    for pair in detected:
                        ...
                        if res:
                            ...
                            os.fsync(f.fileno())
                            found += 1
                            print(f"\n[MATCH] [{res['action']}] {pair} | {res['peak_pct']}% | {message.date.strftime('%Y-%m-%d %H:%M')}")
                        await asyncio.sleep(0.01)

    finally:
        print("\n[SYSTEM] Closing active sessions...")
